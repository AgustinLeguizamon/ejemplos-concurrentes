...
impl TransactionCoordinator {
    ...
    fn submit(&mut self, t: TransactionId) -> bool {
        match self.log.get(&t) {
            ...
            Some(TransactionState::Wait) => self.full_protocol(t, false),
    ...
    fn broadcast_and_wait(&self, message: u8, t: TransactionId, expected: TransactionState) -> bool {
        *self.responses.0.lock().unwrap() = vec![None; STAKEHOLDERS];
        ...
        let responses_guard = self.responses.1.wait_timeout_while(self.responses.0.lock().unwrap(),
                                                                  TIMEOUT,
                                                                  |responses| responses.iter().any(Option::is_none));
        if responses_guard.is_err() {
            println!("[COORDINATOR] timeout {}", t.0);
            false
        } else {
            responses_guard.unwrap().0.iter().all(|opt_transaction_state|opt_transaction_state.is_some() && opt_transaction_state.unwrap() == expected)
        }
    ...
    fn receiver(&mut self) {
        loop {
            ...
            let (read, adrr) = self.socket.recv_from(&mut buf).unwrap();
            let id_from = usize::from_le_bytes(buf[1..].try_into().unwrap());

            match &buf[0] {
                b'C' => {
                    println!("[COORDINATOR] recibí COMMIT de {}", id_from);
                    self.responses.0.lock().unwrap().push(Some(TransactionState::Commit));
                    self.responses.1.notify_all();
                }
                b'A' => {
                    println!("[COORDINATOR] recibí ACCEPT de {}", id_from);
                    self.responses.0.lock().unwrap().push(Some(TransactionState::Abort));
                    self.responses.1.notify_all();
                }
                _ => {
